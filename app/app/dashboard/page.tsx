  ...
  return (
    <div className="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-[96rem] mx-auto">
      ...
      <div className="sm:flex sm:justify-between sm:items-center mb-8">
        ...
        <Link
          href="/app/new"
          className="btn bg-violet-500 hover:bg-violet-600 text-white text-sm shadow-sm"
        >
      ...
      {!projects || projects.length === 0 ? (
        <div className="text-center py-16">
          ...
          <Link href="/app/new" className="btn bg-violet-500 hover:bg-violet-600 text-white text-sm">
          ...
          {projects.map((project) => (
            ...
            >
              <div className="flex items-start justify-between mb-3">
                ...
                <span className={`text-xs font-medium px-2 py-0.5 rounded-full capitalize shrink-0 ml-2 ${stageColors[project.stage] || stageColors.validate}`}>
                  {project.stage}
                </span>
              </div>
              {project.one_liner && (
                <p className="text-sm text-gray-500 dark:text-gray-400 mb-3 line-clamp-2">{project.one_liner}</p>
              )}
              <div className="flex items-center justify-between">
                <div className="text-2xl font-bold text-gray-800 dark:text-gray-100">
                  {project.launch_score}<span className="text-sm font-normal text-gray-400">/100</span>
                </div>
