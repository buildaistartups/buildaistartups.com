...
import { IconChecklist, IconDocument, IconBuild, IconSettings, IconAI } from '@/components/app/icons'
...
const CATEGORIES = [
  { value: 'frontend', label: 'Frontend' }, { value: 'backend', label: 'Backend' }, { value: 'database', label: 'Database' },
  { value: 'hosting', label: 'Hosting' }, { value: 'auth', label: 'Auth' }, { value: 'payments', label: 'Payments' },
  { value: 'analytics', label: 'Analytics' }, { value: 'other', label: 'Other' },
]
...
export default function BuildPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    if (proj) { setGithubUrl(proj.github_url || ''); setProjectName(proj.name || ''); setProjectOneLiner(proj.one_liner || '') }
  }, [projectId])
  ...
  async function saveSpec() {
    setSpecSaving(true)
    await fetch(`/api/projects/${projectId}/spec`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ content: spec }) })
    setSpecSaving(false); setSpecSaved(true); setTimeout(() => setSpecSaved(false), 2000)
  }

  async function addStackItem(e: React.FormEvent) {
    e.preventDefault()
    if (!newStack.name.trim()) return
    await fetch(`/api/projects/${projectId}/stack`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(newStack) })
    setNewStack({ category: 'frontend', name: '', url: '', notes: '' }); fetchData()
  }
  ...
  async function aiRecommendStack() {
    setAiStackLoading(true)
    try {
      const res = await fetch('/api/ai', { method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ type: 'recommend-stack', input: `Project: ${projectName} — ${projectOneLiner}. Solo developer building a micro-SaaS. Need fast shipping, low cost, good scalability.`, projectId }) })
      const data = await res.json()
      if (res.ok && data.result?.recommendations) {
        for (const r of data.result.recommendations) {
          await fetch(`/api/projects/${projectId}/stack`, { method: 'POST', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ category: r.category || 'other', name: r.name, url: r.url || '', notes: r.reason || '' }) })
        }
        fetchData()
      }
    } catch { /* ignore */ }
    finally { setAiStackLoading(false) }
  }

  async function saveGithub() {
    await fetch(`/api/projects/${projectId}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ github_url: githubUrl }) })
    setGithubSaved(true); setTimeout(() => setGithubSaved(false), 2000)
  }

  const completedCount = checklist.filter(i => i.completed).length
  const groupedStack = CATEGORIES.map(c => ({ ...c, items: stack.filter(s => s.category === c.value) })).filter(g => g.items.length > 0)
    ...
    <div className="space-y-6">
      ...
      {tab === 'checklist' && (
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700/60 rounded-xl p-6">
          <TabInfo title="Build Checklist" description="Track your development progress. These are the essential steps to go from idea to working product. Completing items increases your Product score in LaunchScore." />
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="text-right">
              ...
              <div className="w-24 h-2 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden mt-1"><div className="h-full bg-blue-500 rounded-full transition-all" style={{ width: checklist.length > 0 ? `${(completedCount / checklist.length) * 100}%` : '0%' }} /></div>
            </div>
          </div>
          <div className="space-y-1">{checklist.map(item => (
            <label key={item.id} className="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700/30 cursor-pointer transition">
              <input type="checkbox" checked={item.completed} onChange={() => toggleChecklist(item.id, item.completed)} className="form-checkbox rounded text-blue-500 w-5 h-5 border-gray-300 dark:border-gray-600" />
      ...
      {tab === 'spec' && (
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700/60 rounded-xl p-6">
          <TabInfo title="Spec Notes" description="Write your product specification here. A good spec covers: the problem you're solving, your solution, user stories, data model, and what's in vs out of scope for your MVP (Minimum Viable Product — the simplest version worth launching)." />
          <textarea value={spec} onChange={e => setSpec(e.target.value)} rows={20}
          ...
          <button onClick={saveSpec} disabled={specSaving} className="btn bg-blue-500 hover:bg-blue-600 text-white text-sm rounded-xl px-5 py-2.5 disabled:opacity-50">
      ...
      {tab === 'stack' && (
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700/60 rounded-xl p-6">
          <TabInfo title="Tech Stack" description="Track the tools and technologies you're using. Not sure what to pick? Use the AI button to get personalized recommendations based on your project. You can always add or remove items manually." />
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-100">Stack</h2>
            <button onClick={aiRecommendStack} disabled={aiStackLoading}
              className="btn bg-blue-500 hover:bg-blue-600 text-white text-xs rounded-xl px-4 py-2 disabled:opacity-50">
              {aiStackLoading ? <span className="flex items-center gap-2"><svg className="animate-spin w-3 h-3" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" fill="none"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg>Recommending...</span>
                : <span className="flex items-center gap-1.5"><IconAI className="w-3.5 h-3.5" /> AI Recommend Stack</span>}
            </button>
          </div>
          {groupedStack.length > 0 && (
            <div className="space-y-4 mb-6">{groupedStack.map(group => (
              <div key={group.value}>
                <div className="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-2">{group.label}</div>
                <div className="grid sm:grid-cols-2 gap-2">{group.items.map(item => (
          ...
          <form onSubmit={addStackItem} className="space-y-3 border-t border-gray-200 dark:border-gray-700/60 pt-4">
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              <select value={newStack.category} onChange={e => setNewStack(p => ({ ...p, category: e.target.value }))} className={inputClasses.replace('form-input', 'form-select')}>{CATEGORIES.map(c => <option key={c.value} value={c.value}>{c.label}</option>)}</select>
      ...
      {tab === 'github' && (
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700/60 rounded-xl p-6">
          <TabInfo title="GitHub Repository" description="Link your code repository for quick access. This is optional but helps keep everything in one place." />
          <div className="flex gap-3">
            <input value={githubUrl} onChange={e => setGithubUrl(e.target.value)} placeholder="https://github.com/username/repo" className={inputClasses + " flex-1"} />
            <button onClick={saveGithub} className="btn bg-blue-500 hover:bg-blue-600 text-white text-sm rounded-xl px-5 py-2.5 shrink-0">{githubSaved ? '✓ Saved!' : 'Save'}</button>
          </div>
          {githubUrl && (
            <a href={githubUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 mt-4 text-sm text-violet-500 hover:text-violet-600">
