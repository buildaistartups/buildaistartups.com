...
export default function ValidatePage() {
  ...
  useEffect(() => {
    async function loadCachedAnalysis() {
      try {
        ...
        if (res.ok) { const data = await res.json(); if (data && data.result) { setAiResult(data.result); setAiInput(data.input || '') } }
      } catch { /* no cache */ }
    }
    loadCachedAnalysis()
    fetchData()
  ...
  async function runAI() {
    ...
    try {
      const res = await fetch('/api/ai', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ type: 'analyze-idea', input: aiInput, projectId }) })
  ...
  async function aiDiscoverCompetitors() {
    if (!aiInput.trim() && !icp.persona) return
    setAiCompLoading(true)
    try {
      const desc = aiInput || `Startup targeting ${icp.persona} with pain points: ${icp.pain_points}`
      const res = await fetch('/api/ai', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ type: 'find-competitors', input: desc, projectId }) })
      const data = await res.json()
      if (!res.ok) return
      if (data.result?.competitors) {
        for (const c of data.result.competitors) {
          await fetch(`/api/projects/${projectId}/competitors`, {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ name: c.name, url: c.url || '', pricing: c.pricing || '', strengths: c.strengths || [], weaknesses: c.weaknesses || [], market_share: c.market_share || 'emerging' }),
          })
        }
        fetchData()
      }
    } catch { /* ignore */ }
    finally { setAiCompLoading(false) }
  }
  ...
  function downloadPDF() {
    ...
    printWindow.document.write(`<!DOCTYPE html><html><head><title>AI Analysis Report</title>
      ...
      <p class="subtitle">Generated by LaunchScore &middot; ${new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</p>
      <div class="badge ${aiResult.verdict}">${aiResult.verdict.replace('-', ' ').toUpperCase()} &mdash; ${aiResult.score}/100</div>
      <h2>Market Analysis</h2>
      <div class="grid"><div class="card"><div class="card-label">TAM</div><p>${aiResult.marketAnalysis.tam}</p></div><div class="card"><div class="card-label">Growth</div><p>${aiResult.marketAnalysis.growth}</p></div><div class="card"><div class="card-label">Timing</div><p>${aiResult.marketAnalysis.timing}</p></div></div>
      ...
      <div class="grid"><div class="card"><div class="card-label">Persona</div><p>${aiResult.icp.persona}</p></div><div class="card"><div class="card-label">Pain Point</div><p>${aiResult.icp.pain}</p></div><div class="card"><div class="card-label">Budget</div><p>${aiResult.icp.budget}</p></div></div>
      <h2>Key Risks</h2>${aiResult.risks.map((r, i) => `<div class="item"><span class="num num-amber">${i + 1}</span><p>${r}</p></div>`).join('')}
      <h2>Recommended Next Steps</h2>${aiResult.nextSteps.map((s, i) => `<div class="item"><span class="num">${i + 1}</span><p>${s}</p></div>`).join('')}
      <div class="footer">LaunchScore &middot; buildaistartups.com</div>
      </body></html>`)
  ...
  async function addCompetitor(e: React.FormEvent) {
    ...
    await fetch(`/api/projects/${projectId}/competitors`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...newComp, strengths: newComp.strengths.split(',').map(s => s.trim()).filter(Boolean), weaknesses: newComp.weaknesses.split(',').map(s => s.trim()).filter(Boolean) }) })
    ...
    <div className="space-y-6">
      ...
      {tab === 'checklist' && (
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700/60 rounded-xl p-6">
          <TabInfo title="Validation Checklist" description="Complete these tasks to validate your idea before investing time building. Each completed item strengthens your Market score in LaunchScore. The goal is evidence that real people want what you're building." />
      ...
      {tab === 'ai' && (
        <div className="space-y-6">
          <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700/60 rounded-xl p-6">
            <TabInfo title="AI Idea Analysis" description="Describe your startup idea in plain language. Our AI will evaluate market size, competition, ideal customer, risks, and give you an honest score out of 100. You can write in any language." />
            <div className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 rounded-xl bg-violet-500/10 flex items-center justify-center"><IconAI className="w-5 h-5 text-violet-500" /></div>
              <div>
                <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-100">AI Idea Analyzer</h2>
              </div>
            </div>
            <textarea value={aiInput} onChange={e => setAiInput(e.target.value)} rows={4} placeholder="Describe your startup idea here... Be specific about what it does, who it's for, and how it makes money."
          ...
          {aiResult && (
            <div ref={reportRef} className="space-y-4">
              ...
              <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700/60 rounded-xl p-6">
                <h3 className="text-sm font-semibold text-gray-800 dark:text-gray-100 uppercase tracking-wider mb-4 flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-violet-500"></span> Market Analysis</h3>
                <div className="grid sm:grid-cols-3 gap-4">
                  {[{ key: 'tam', label: 'Total Addressable Market', Icon: IconTarget }, { key: 'growth', label: 'Growth Trajectory', Icon: IconTrend }, { key: 'timing', label: 'Market Timing', Icon: IconClock }].map(item => (
                    <div key={item.key} className="bg-gray-50 dark:bg-gray-700/40 rounded-xl p-4 border border-gray-100 dark:border-gray-700/30">
                      <div className="flex items-center gap-2 mb-2"><item.Icon className="w-4 h-4 text-violet-500" /><span className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">{item.label}</span></div>
              ...
              <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700/60 rounded-xl p-6">
                <h3 className="text-sm font-semibold text-gray-800 dark:text-gray-100 uppercase tracking-wider mb-4 flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-violet-500"></span> Competitive Landscape</h3>
                <div className="grid sm:grid-cols-2 gap-3">
                  {aiResult.competitors.map((c, i) => (
                    <div key={i} className="bg-gray-50 dark:bg-gray-700/40 rounded-xl p-4 border border-gray-100 dark:border-gray-700/30">
                      <div className="font-semibold text-sm text-gray-800 dark:text-gray-100 mb-2 flex items-center gap-2"><IconCompetitors className="w-4 h-4 text-gray-400" /> {c.name}</div>
                      <div className="flex items-start gap-2 mb-1.5"><IconArrowUp className="w-3 h-3 text-emerald-500 mt-0.5 shrink-0" /><span className="text-xs text-gray-600 dark:text-gray-300 leading-relaxed">{c.strength}</span></div>
                      <div className="flex items-start gap-2"><IconArrowDown className="w-3 h-3 text-red-400 mt-0.5 shrink-0" /><span className="text-xs text-gray-600 dark:text-gray-300 leading-relaxed">{c.weakness}</span></div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700/60 rounded-xl p-6">
                <h3 className="text-sm font-semibold text-gray-800 dark:text-gray-100 uppercase tracking-wider mb-4 flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-violet-500"></span> Ideal Customer Profile</h3>
                <div className="grid sm:grid-cols-3 gap-4">
                  <div className="bg-gray-50 dark:bg-gray-700/40 rounded-xl p-4 border border-gray-100 dark:border-gray-700/30"><div className="flex items-center gap-2 mb-2"><IconUser className="w-4 h-4 text-violet-500" /><span className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase">Persona</span></div><p className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed">{aiResult.icp.persona}</p></div>
                  <div className="bg-gray-50 dark:bg-gray-700/40 rounded-xl p-4 border border-gray-100 dark:border-gray-700/30"><div className="flex items-center gap-2 mb-2"><IconFire className="w-4 h-4 text-orange-500" /><span className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase">Pain Point</span></div><p className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed">{aiResult.icp.pain}</p></div>
                  <div className="bg-gray-50 dark:bg-gray-700/40 rounded-xl p-4 border border-gray-100 dark:border-gray-700/30"><div className="flex items-center gap-2 mb-2"><IconCurrency className="w-4 h-4 text-emerald-500" /><span className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase">Budget</span></div><p className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed">{aiResult.icp.budget}</p></div>
                </div>
              </div>

              <div className="grid sm:grid-cols-2 gap-4">
                <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700/60 rounded-xl p-6">
                  <h3 className="text-sm font-semibold text-gray-800 dark:text-gray-100 uppercase tracking-wider mb-4 flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-amber-500"></span> Key Risks</h3>
                  <div className="space-y-3">{aiResult.risks.map((r, i) => (<div key={i} className="flex items-start gap-3"><div className="w-5 h-5 rounded-md bg-amber-500/10 flex items-center justify-center shrink-0 mt-0.5"><IconWarning className="w-3 h-3 text-amber-500" /></div><p className="text-sm text-gray-600 dark:text-gray-300 leading-relaxed">{r}</p></div>))}</div>
                </div>
                <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700/60 rounded-xl p-6">
                  <h3 className="text-sm font-semibold text-gray-800 dark:text-gray-100 uppercase tracking-wider mb-4 flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-emerald-500"></span> Next Steps</h3>
                  <div className="space-y-3">{aiResult.nextSteps.map((s, i) => (<div key={i} className="flex items-start gap-3"><div className="w-5 h-5 rounded-full bg-violet-500 flex items-center justify-center shrink-0 mt-0.5"><span className="text-white text-[10px] font-bold">{i + 1}</span></div><p className="text-sm text-gray-600 dark:text-gray-300 leading-relaxed">{s}</p></div>))}</div>
                </div>
              </div>
            </div>
          )}
      ...
      {tab === 'competitors' && (
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700/60 rounded-xl p-6">
          <TabInfo title="Competitor Research" description="Track your direct and indirect competitors. Understanding their strengths and weaknesses helps you find gaps in the market. Use the AI button to auto-discover top competitors, or add them manually." />
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-100">Competitors</h2>
            <button onClick={aiDiscoverCompetitors} disabled={aiCompLoading}
              className="btn bg-violet-500 hover:bg-violet-600 text-white text-xs rounded-xl px-4 py-2 disabled:opacity-50">
              {aiCompLoading ? <span className="flex items-center gap-2"><svg className="animate-spin w-3 h-3" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" fill="none"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg>Finding...</span>
                : <span className="flex items-center gap-1.5"><IconAI className="w-3.5 h-3.5" /> AI Find Competitors</span>}
            </button>
          </div>
            ...
            <div className="grid sm:grid-cols-2 gap-3 mb-6">
              {competitors.map(c => (
                <div key={c.id} className="bg-gray-50 dark:bg-gray-700/40 rounded-xl p-4 border border-gray-100 dark:border-gray-700/30">
                  <div className="flex items-start justify-between mb-2"><div className="font-semibold text-sm text-gray-800 dark:text-gray-100">{c.name}</div><button onClick={() => deleteCompetitor(c.id)} className="text-xs text-red-400 hover:text-red-500">✕</button></div>
          ...
          <form onSubmit={addCompetitor} className="space-y-3 border-t border-gray-200 dark:border-gray-700/60 pt-4">
            <div className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Add manually</div>
            <div className="grid sm:grid-cols-2 gap-3">
      ...
      {tab === 'icp' && (
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700/60 rounded-xl p-6">
          <TabInfo title="Ideal Customer Profile (ICP)" description="Your ICP defines exactly who would pay for your product. ICP stands for 'Ideal Customer Profile' — a detailed description of the person or company most likely to buy. A clear ICP helps you focus marketing, prioritize features, and avoid building for the wrong audience." />
          <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-100 mb-4">Define your ICP</h2>
      ...
      {tab === 'demand' && (
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700/60 rounded-xl p-6">
          <TabInfo title="Demand Signals" description="Track real evidence that people want your product. Demand signals include waitlist signups, letters of intent (LOI — a written commitment from a potential customer), preorders, and survey responses. More signals = stronger Market score." />
          <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-100 mb-4">Signals</h2>
          {demand.length > 0 && (
