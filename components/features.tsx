...
export default function Features() {
  ...
  useEffect(() => {
    ...
    if (!maybe) return
    const ctx = maybe as CanvasRenderingContext2D // <- narrowed, not nullable

    let w = 0, h = 0, cx = 0, cy = 0, t = 0
    const dpr = Math.min(window.devicePixelRatio || 1, 2)

    type P = { baseR: number; r: number; a: number; speed: number; size: number; hue: number; tw: number }
    let particles: P[] = []
    let raf = 0
    ...
    function resize() {
      ...
      particles = Array.from({ length: 200 }, () => {
        const r = Math.pow(Math.random(), 0.8) * Math.min(w, h) * 0.35 + 20 * dpr
        return {
          baseR: r,
          r,
          a: Math.random() * Math.PI * 2,
          speed: (Math.random() * 0.0007 + 0.0002) * (Math.random() < 0.5 ? -1 : 1),
          size: Math.random() * 1.4 * dpr + 0.6 * dpr,
      ...
      <div className="relative max-w-6xl mx-auto px-4 sm:px-6">
        ...
        <div className="absolute inset-0 -z-10 -mx-28 rounded-t-[3rem] pointer-events-none overflow-hidden" aria-hidden="true">
          <div className="absolute left-1/2 -translate-x-1/2 top-0 -z-10">
            <Image src={Illustration} className="max-w-none" width={1404} height={658} alt="Features background glow" />
          </div>
        </div>
            ...
            <div className="max-w-xl mx-auto md:max-w-none flex flex-col md:flex-row md:space-x-8 lg:space-x-16 xl:space-x-20 space-y-8 space-y-reverse md:space-y-0">
              ...
              <div className="md:w-7/12 lg:w-1/2 order-1 md:order-none max-md:text-center" data-aos="fade-down">
                ...
                <p className="feature-subtext text-lg mb-8 dark:text-slate-400">
                  Describe your idea in a sentence. Build AI Startups researches the niche, drafts the product spec, scaffolds a
                  production-ready repo, ships the UI &amp; docs, wires auth/billing/analytics, and deploys. You keep everything
                  under your own GitHub, Vercel, and Stripe.
                </p>
              ...
              {/* Orb visual (right column) */}
              <div className="md:w-5/12 lg:w-1/2" data-aos="fade-up" data-aos-delay="100">
                <div className="relative -mt-12">
                  <div ref={sceneRef} className="bao-scene">
                    <div className="bao-halo" aria-hidden="true" />
                    <canvas ref={canvasRef} className="bao-field" />

                    <div className="bao-orb">
                      <div className="bao-ring r1" />
                      <div className="bao-ring r2" />
                      <div className="bao-ring r3" />

                      <div className="bao-logo-plate">
                        <div className="bao-brand">
                          Build AI Startups
                          <small>From one-line spec to shipped product</small>
                        </div>
                      </div>

                      <div className="bao-chip c1"><span>Spec → PRD</span></div>
                      <div className="bao-chip c2"><span>Repo / UI</span></div>
                      <div className="bao-chip c3"><span>Deploy</span></div>

                      <svg className="bao-signals" viewBox="0 0 1000 1000" preserveAspectRatio="none">
                        <defs>
                          <linearGradient id="bao-grad" x1="0" y1="0" x2="1" y2="1">
                  ...
                  <style jsx>{`
                    .bao-scene{
                      ...
                      background: radial-gradient(600px 400px at 50% 30%, #0e1222 0%, #0b0f1a 40%, #05070d 100%);
                    }
                    @media (min-width:640px){ .bao-scene{ height:24rem } }
                    @media (min-width:768px){ .bao-scene{ height:26rem } }

                    .bao-halo{
                      position:absolute; inset:-15%;
                      background:
                        radial-gradient(45% 55% at 50% 40%, rgba(20,184,166,0.10), transparent 60%),
                        radial-gradient(65% 55% at 50% 60%, rgba(99,102,241,0.10), transparent 60%),
                        conic-gradient(from 0deg, rgba(168,85,247,0.18), rgba(34,211,238,0.18), rgba(124,58,237,0.18), rgba(20,184,166,0.18), rgba(168,85,247,0.18));
                      filter: blur(22px);
                      animation: bao-spin 38s linear infinite;
                      opacity:.75;
                      pointer-events:none;
                    }
                    @keyframes bao-spin{ to{ transform: rotate(1turn) } }
                    ...
                    .bao-orb{
                      ...
                      border-radius:50%;
                      background:
                        radial-gradient(120px 120px at 60% 35%, rgba(255,255,255,0.35), transparent 60%),
                      ...
                      box-shadow:
                        ...
                        inset 0 0 160px rgba(34,211,238,0.22),
                    ...
                    .bao-orb::before{
                      ...
                      border-radius:50%;
                      background:
                        radial-gradient(120% 80% at 30% 20%, rgba(168,85,247,0.25), transparent 50%),
                    ...
                    .bao-ring::before, .bao-ring::after{
                      content:""; position:absolute; inset:0; border-radius:inherit;
                      box-shadow: 0 0 0 1px rgba(168,85,247,0.55), 0 0 40px rgba(168,85,247,0.25) inset;
                      mix-blend-mode:screen;
                    }
                    .bao-ring.r1{ animation: bao-tilt1 16s linear infinite }
                    .bao-ring.r2{ animation: bao-tilt2 21s linear infinite reverse }
                    .bao-ring.r3{ animation: bao-tilt3 27s linear infinite }
                    @keyframes bao-tilt1{ to{ transform: rotateX(65deg) rotateZ(360deg) } }
                    @keyframes bao-tilt2{ to{ transform: rotateY(65deg) rotateZ(-360deg) } }
                    @keyframes bao-tilt3{ to{ transform: rotateX(20deg) rotateY(30deg) rotateZ(360deg) } }
                    ...
                    .bao-chip{
                      ...
                      font-size:12px; color:#cbd5e1; letter-spacing:.2px;
                    ...
                    .bao-chip.c1{ animation: bao-orbit1 12s linear infinite }
                    .bao-chip.c2{ animation: bao-orbit2 18s linear infinite reverse }
                    .bao-chip.c3{ animation: bao-orbit3 22s linear infinite }
                    @keyframes bao-orbit1{ from{ transform:rotate(0deg) translateX(var(--r)) rotate(0deg)} to{ transform:rotate(360deg) translateX(var(--r)) rotate(-360deg)} }
                    @keyframes bao-orbit2{ from{ transform:rotate(0deg) translateX(calc(var(--r) + 40px)) rotate(0deg)} to{ transform:rotate(360deg) translateX(calc(var(--r) + 40px)) rotate(-360deg)} }
                    @keyframes bao-orbit3{ from{ transform:rotate(0deg) translateX(calc(var(--r) + 80px)) rotate(0deg)} to{ transform:rotate(360deg) translateX(calc(var(--r) + 80px)) rotate(-360deg)} }

                    .bao-logo-plate{
                      position:relative; width:64%; aspect-ratio:3.6/1.2; border-radius:22px;
                    ...
                    .bao-logo-plate::before{
                      content:""; position:absolute; inset:-20%;
                      background: conic-gradient(from 0deg, rgba(168,85,247,0.25), rgba(34,211,238,0.18), rgba(99,102,241,0.25), rgba(20,184,166,0.18), rgba(168,85,247,0.25));
                      filter: blur(32px); animation: bao-spin 30s linear infinite; opacity:.7; pointer-events:none;
                    }
                    .bao-brand{
                      font-weight:800;
                    ...
                    .bao-brand small{
                      display:block; font-weight:600; margin-top:6px;
                      font-size: clamp(11px, 1.2vw, 12.5px); letter-spacing:.4px;
                    ...
                    .bao-signals path{
                      fill:none; stroke:url(#bao-grad); stroke-width:1.2; stroke-linecap:round;
                      stroke-dasharray:3 10; animation: bao-dash 3.2s linear infinite; opacity:.8;
                    }
                    @keyframes bao-dash{ to { stroke-dashoffset: -200 } }
                  `}</style>
                </div>
              </div>
              {/* /Orb visual */}
