...
const mentorCategories = [
  ...
  { icon: <TrendingUp />, title: 'Growth Experts', count: 423, color: 'green' },
  { icon: <Code />, title: 'Technical CTOs', count: 892, color: 'blue' },
  { icon: <DollarSign />, title: 'Fundraising Veterans', count: 485, color: 'yellow' }
]
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        <div className="py-12 md:py-20">
          ...
          <div className="text-center mb-12">
            ...
            <div className="text-lg text-slate-400">Verified Mentors</div>
          </div>
          ...
          <div className="grid md:grid-cols-4 gap-6 mb-12">
            {mentorCategories.map((category) => (
              <div key={category.title} className="bg-slate-800/50 rounded-xl p-6 text-center">
                <div className={`inline-flex p-3 bg-${category.color}-500/20 rounded-lg mb-4`}>
                  {React.cloneElement(category.icon, { className: `w-8 h-8 text-${category.color}-400` })}
                </div>
                <div className="text-2xl font-bold text-slate-200 mb-1">{category.count}</div>
                <div className="text-sm text-slate-400">{category.title}</div>
              </div>
            ))}
          </div>
