...
const traits = [
  ...
  },
  {
    ...
    title: 'Energetic',
    text: 'You bring energy and curiosity to every project and thrive in fast-paced environments.',
  },
  {
    icon: (
    ...
    title: 'Achiever',
    text: 'You set ambitious goals, deliver, and help others reach new heights too.',
  },
]

const images = [Recruitment01, Recruitment02, Recruitment03, Recruitment04]

const items: Item[] = [
  {
    area: 'Engineering',
    positions: [
      { title: 'Software Engineering Intern', location: 'Remote / North America', link: '#0' },
      { title: 'Software Engineer, Core Infrastructure', location: 'Remote', link: '#0' },
      { title: 'Software Engineer, Data Platform', location: 'Remote / Europe', link: '#0' },
      { title: 'Software Engineer, Marketing Systems', location: 'Remote / North America', link: '#0' },
    ]
  },
  {
    area: 'Marketing',
    positions: [
      { title: 'Product Marketing Manager', location: 'Remote / North America', link: '#0' },
      { title: 'Marketing Manager', location: 'Remote / Europe', link: '#0' },
    ]
  },
  {
    area: 'Design',
    positions: [
      { title: 'Product Designer, Consumer', location: 'Remote / North America', link: '#0' },
      { title: 'Senior Product Designer', location: 'Remote / Europe', link: '#0' },
      { title: 'Lead Designer', location: 'Remote / Europe', link: '#0' },
    ]
  },
]
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        <div className="py-12 md:py-20 border-t [border-image:linear-gradient(to_right,transparent,var(--color-slate-800),transparent)1]">
          ...
          <div className="max-w-3xl mx-auto text-center pb-12 md:pb-20">
            <h2 className="h2 bg-clip-text text-transparent bg-linear-to-r from-slate-200/60 via-slate-200 to-slate-200/60 pb-4">Join a team of makers</h2>
          ...
          <div className="grid md:grid-cols-3 gap-6 lg:gap-10 max-w-xs mx-auto md:max-w-none -m-5">
            {traits.map((trait, i) => (
              <div className="text-center p-5" key={i}>
                ...
                <div className="font-bold text-slate-100 mb-1">{trait.title}</div>
                <p className="text-sm text-slate-400">{trait.text}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="relative px-4 sm:px-6">
        ...
        <div className="max-w-3xl mx-auto py-12 md:py-20">
          ...
          <div className="space-y-8">
            {items.map((item, i) => (
              <div key={i}>
                <h4 className="inline-flex font-medium bg-clip-text text-transparent bg-linear-to-r from-purple-500 to-purple-200 pb-3">{item.area}</h4>
                <ul className="divide-y divide-slate-800">
                  {item.positions.map((position, j) => (
                    <li key={j} className="flex flex-col md:flex-row flex-wrap justify-between py-5">
                      <span className="font-medium text-slate-50 mr-2">{position.title}</span>
                      <a className="inline-flex text-sm font-medium text-purple-500 group" href={position.link}>
                        {position.location}
                        <span className="tracking-normal group-hover:translate-x-0.5 transition-transform duration-150 ease-in-out ml-1">&rarr;</span>
                      </a>
                    </li>
                  ))}
                </ul>
