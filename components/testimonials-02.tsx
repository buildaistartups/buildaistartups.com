        ...
        <div className="py-12 md:py-20">
          ...
          <div className="grid lg:grid-cols-3 gap-4 sm:gap-6 max-w-xs mx-auto lg:max-w-none">
            {items.map((item, idx) => (
              <div
                key={item.name}
                className="relative p-5 before:absolute before:inset-0 before:-z-10 before:border before:border-slate-300 before:bg-slate-700 before:opacity-10 before:rounded-xl rounded-xl hover:shadow-lg transition-shadow duration-200"
              >
                <div className="flex items-center justify-between mb-4">
                  ...
                  <a
                    className={`shrink-0 text-slate-500 hover:text-purple-500 transition-colors duration-150
                    ${item.twitter === '#0' ? 'pointer-events-none opacity-30' : ''}`}
                    href={item.twitter}
